...
6. Prefer clips with non-empty `news_id`, exact `game_id`, exact player/team/action matches, higher `item_info.star`, and shorter duration.
7. If exact clips are unavailable, use fallback clips and explain in `warnings`.
8. Return the JSON contract only.
